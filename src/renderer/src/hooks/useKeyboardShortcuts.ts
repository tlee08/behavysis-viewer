  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          e.preventDefault()
          state.setCurrentFrame(Math.max(0, currentFrame - Math.round(jumpSeconds * fps)))
          break
        case 'ArrowRight':
          e.preventDefault()
          state.setCurrentFrame(Math.min(numFrames - 1, currentFrame + Math.round(jumpSeconds * fps)))
          break
        case 'k':
        case 'K':
        ...
        case 'R': {
          ...
          if (bout) state.setCurrentFrame(Math.max(0, bout.start - focusSizeFrames))
          break
        }
        case 'ArrowUp':
        case 'ArrowDown': {
        ...
        case '1':
          if (selectedBoutId !== null) state.updateBoutActual(selectedBoutId, 1)
          break
        case '2':
          if (selectedBoutId !== null) state.updateBoutActual(selectedBoutId, 0)
          break
        case '3':
          if (selectedBoutId !== null) state.updateBoutActual(selectedBoutId, -1)
          break
