  ...
  for (let i = 0; i < numFrames; i++) {
    if (preds[i] === 1 && runStart === -1) {
      ...
      bouts.push({
        ...
        stop: i - 1 + offset,
        behav,
        actual: clampActual(actuals[runStart]),
 ...
 */
export function parseBehavParquet(path: string): { bouts: Bout[]; numFrames: number } {
  ...
  for (const behav of behavNames) {
    ...
    const preds = Int8Array.from(df.getColumn(predName).toArray(), (v) => Number(v))
    ...
    for (const { arrowName, behav: b, col } of behavCols) {
      if (b === behav && col !== 'actual') {
        userDefCols.set(col, Int8Array.from(df.getColumn(arrowName).toArray(), (v) => Number(v)))
      }
    }

    allBouts.push(...framesToBouts(preds, behav, actuals, userDefCols, row0Frame))
  }
